"""
Unit and regression test for the demokit package.
...
def test_demokit_imported():
    """Sample test, will always pass so long as import statement worked"""
    assert "demokit" in sys.modules
...
class TestRMSF(object):
    ...
    def test_rmsf(self, universe):
        rmsfs = demokit.RMSF(universe.select_atoms('name CA'))
        rmsfs.run()
    ...
    def test_rmsf_single_frame(self, universe):
        rmsfs = demokit.RMSF(universe.select_atoms('name CA')).run(start=5, stop=6)
